    ...
    function showInvestmentSurvey(eventId) {
        ...
        submitBtn.addEventListener('click', () => {
            if (submitBtn.classList.contains('enabled')) {
                ...
                if(contentEl) contentEl.innerHTML = `<div style="font-size: 16px; color: #555; padding: 100px 20px;">Saving your preferences...</div>`;
            }
        });
    ...
    function showInvestmentRecommendation(profile, eventId) {
        const promptContent = document.getElementById('pw-content-area');
        if (!promptContent) {
            const overlay = document.createElement('div');
            overlay.id = 'pocketwisely-prompt-overlay';
            overlay.innerHTML = `<div id="pocketwisely-prompt"><div id="pw-content-area"></div></div>`;
            document.body.appendChild(overlay);
            const style = document.createElement('style');
            style.textContent = `#pocketwisely-prompt-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(10,20,30,.5);backdrop-filter:blur(5px);z-index:2147483647;display:flex;align-items:center;justify-content:center;padding:40px 15px;animation:overlayFadeIn .3s ease-out}#pocketwisely-prompt{background:#fff;border-radius:20px;box-shadow:0 20px 60px -10px rgba(0,0,0,.25);width:100%;max-width:500px;text-align:center;border:0;animation:promptSlideIn .4s cubic-bezier(.34,1.56,.64,1);max-height:calc(100vh - 80px);overflow:hidden;display:flex; padding: 28px;}`;
            document.head.appendChild(style);
        }
        const finalContentArea = document.getElementById('pw-content-area');
        ...
        const duration = profile.investment_duration;

        if (risk === 'low' && duration === 'short') { recommendationText = 'Fixed Deposits or Government Bonds'; }
        else if (risk === 'medium' && duration === 'medium') { recommendationText = 'Balanced Mutual Funds or Index Funds'; }
        else if (risk === 'high' && duration === 'long') { recommendationText = 'Equity Mutual Funds or Growth Stocks'; }
        else { recommendationText = 'a Diversified Investment Portfolio'; }

        const recommendationHTML = `
            <div style="text-align: center; width: 100%;">
                <h2 style="font-size: 20px; color: #1e293b; margin-bottom: 10px;">Your Investment Path</h2>
                <p style="font-size: 14px; color: #64748b; margin-bottom: 20px;">Based on your profile (<b>${capitalize(risk)} Risk</b>, <b>${capitalize(duration)} Term</b>), we suggest:</p>
                <p style="font-size: 18px; font-weight: bold; color: #14b8a6; margin-bottom: 25px;">${recommendationText}</p>
                <div class="pw-button-container" style="display:flex; gap:12px; flex-direction:column;">
                    <button id="pw-explore-investing" class="pw-invest-btn">Explore Investment Options</button>
                    <button id="pw-change-prefs" class="pw-proceed-btn">Change Preferences</button>
                </div>
            </div>
            <style>
                .pw-invest-btn{background:#2dd4bf;color:#fff;border:none;padding:16px 20px;border-radius:12px;font-size:16px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:all .2s ease;box-shadow:0 4px 15px -2px rgba(45,212,191,.3)}.pw-invest-btn:hover{background:#14b8a6;transform:translateY(-2px);box-shadow:0 6px 20px -2px rgba(45,212,191,.4)}.pw-proceed-btn{background:#f3f4f6;color:#4b5563;border:none;padding:14px 20px;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;transition:all .2s ease}.pw-proceed-btn:hover{background:#e5e7eb}
            </style>
        `;
        finalContentArea.innerHTML = recommendationHTML;

        document.getElementById('pw-change-prefs').addEventListener('click', () => {
            chrome.runtime.sendMessage({ action: "showInvestmentSurvey", eventId: eventId });
        });

        document.getElementById('pw-explore-investing').addEventListener('click', () => {
            alert("Redirecting to our partner investment platform...");
            document.getElementById('pocketwisely-prompt-overlay')?.remove();
        });
    }
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        (async () => {
            ...
            if (request.action === "purchaseAttempt") {
                ...
                if (profileResponse.ok) {
                    ...
                    if (profileData.risk_tolerance) { 
                        chrome.scripting.executeScript({ target: { tabId: tab.id }, func: showInvestmentRecommendation, args: [profileData, request.eventId] });
                    } else { 
                        chrome.scripting.executeScript({ target: { tabId: tab.id }, func: showInvestmentSurvey, args: [request.eventId] });
                    }
                } else { 
                    chrome.scripting.executeScript({ target: { tabId: tab.id }, func: showInvestmentSurvey, args: [request.eventId] });
                }
            ...
            else if (request.action === "saveUserProfile") {
                ...
                const newProfileData = { 
                    risk_tolerance: request.answers.risk_level, 
                    investment_duration: request.answers.duration, 
                    financial_status: request.answers.financial_stability 
                };
                chrome.scripting.executeScript({ target: { tabId: tab.id }, func: showInvestmentRecommendation, args: [newProfileData, request.eventId] });
            }
